...
//				prevForRemove = nullptr;
			...
			void loadModel(const std::string& modelName)
			{
				...
				const aiScene* scene = importer.ReadFile(modelName, aiProcess_Triangulate | aiProcess_JoinIdenticalVertices | aiProcess_SortByPType| aiProcess_PreTransformVertices);
				const aiMesh* mesh = scene->mMeshes[0];

				std::cout << "Vertices count " << mesh->mNumVertices << "\n";

				minBox = glm::vec3(50000);
				maxBox = glm::vec3(-590000);
				...
				for (int i = 0; i < mesh->mNumVertices; ++i)
				{
					...
					for (int i = 0; i < 3; ++i)
					{
						minBox[i] = position[i] < minBox[i] ? position[i] : minBox[i];
						maxBox[i] = position[i] > maxBox[i] ? position[i] : maxBox[i];
					}

					scaleFactor = (maxBox - minBox);

					float s = scaleFactor.x;
					if (scaleFactor.y > s)
					{
						s = scaleFactor.y;
					}
			...
			void saveModel(const std::string& modelName)
			{
				...
				m->mNumFaces = triangles.size();
				m->mFaces = new aiFace[triangles.size()];
				for (int i{ 0 }; i < vertices.size(); ++i)
				{
					m->mVertices[i] = aiVector3D(positionSForDrawing[i].x, positionSForDrawing[i].y, positionSForDrawing[i].z);
					m->mNormals[i] = aiVector3D(normalsForDrawing[i].x, normalsForDrawing[i].y, normalsForDrawing[i].z);
				}
				for (int i{ 0 }; i < triangles.size(); ++i)
				{
					...
					for (int j = 0; j < 3; ++j)
					{
						f.mIndices[j] = indicesForDrawing[3 * i + j];
					}

					m->mFaces[i] = f;
				}
				Assimp::Exporter exp;
				exp.Export(scene, "obj", modelName.c_str());
			...
			}//How many vertices must be not reduced from model.
